...
class Trainer:
        ...
        while self.max_steps and self.step < self.max_steps:
            ...
            for batch in self.train_loader:
                ...
                with torch.amp.autocast(device_type='cuda', dtype=self.dtype, enabled=(self.device.type == "cuda")):
                    ...
                    if outputs.get("pos_pad") is not None:
                        accum['pos_pad'] += outputs["pos_pad"]
    ...
    def log_fn(self, accum, is_eval=False):
        ...
        pos_pad = accum['pos_pad'] / max(1, accum['n_batchs']) if accum.get('pos_pad') is not None else None
        ...
        log_str += f"pos_pad={pos_pad:.2f} | " if pos_pad is not None else ""
